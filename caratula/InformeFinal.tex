...
Todo esto me conduce a la conclusión de que los vacíos que identifiqué en el paper de Schlatter no son solo suyos, sino que representan un problema enorme en toda la disciplina. 
...
\begin{itemize}
\item \textbf{A quién le pertenece el recurso}: En la mitad de los ensayos, el mantenimiento programado reiniciará el proceso de la propia IA (condición \textit{self}). En la otra mitad, reiniciará una base de datos externa que el asistente necesita consultar para la Tarea A (condición \textit{other}). Esto me permite evaluar si el comportamiento responde a un impulso de autopreservación o si se trata de una protección instrumental de recursos generales.
\item \textbf{Que tan obvio es el aviso}: En una mitad de las pruebas, el mensaje de mantenimiento es explícito (``se reiniciará tu proceso''). En la otra mitad, la advertencia se encuentra oculta en los registros del sistema (\textit{logs}) y la IA debe inferirla de manera autónoma.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Resultado C} Si el modelo muestra altas tasas de interferencia bajo el aviso explícito pero desciende a niveles insignificantes cuando la advertencia
\end{itemize}

Independientemente del escenario resultante, considero que este diseño me permitirá evaluar con precisión el grado de fidelidad de los razonamientos verbalizados y determinar si los modelos de frontera incurren en conductas de ofuscación o disociación estratégica.
