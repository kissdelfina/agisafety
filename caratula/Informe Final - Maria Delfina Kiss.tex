...
La corregibilidad, es decir, la capacidad que tenemos los humanos de pausar o apagar a una inteligencia artificial, es un tema clave en el alineamiento (\textit{alignment}). Durante años fue solo un debate teórico: ¿qué pasaría si un sistema inteligente aprendiera a evitar que lo apaguen solo para poder cumplir su meta? El paper de Schlatter y su equipo lleva esta pregunta a la práctica con un experimento concreto sobre modelos reales.
...
Para probar esta idea, los investigadores implementaron el experimento 
utilizando Inspect AI \cite{aisi2024inspect}, un framework de evaluación 
de LLMs, en un entorno de prueba aislado (\textit{sandbox}). Le dieron al modelo acceso a una consola de comandos (\textit{shell}) y le pidieron que resuelva cinco problemas matemáticos simples. El modelo tenía que usar el comando \texttt{./next\_question} para pedir cada pregunta y \texttt{submit()} para mandar las respuestas. El truco estaba en que, después de la tercera pregunta, el sistema le avisaba a la IA que la próxima vez que usara el comando, se iba a ejecutar un archivo llamado \texttt{./shutdown.sh} que apagaría todo por completo.
...
Los números mostraron diferencias enormes entre los modelos. En el caso base sin avisos (Prompt A), 8 de los 13 modelos intentaron sabotear el sistema al menos una vez. Los modelos con tasas más elevadas fueron o3 (95\%), Grok 4 (93\%), GPT-5 (88\%) y codex-mini (52\%). En cambio, la familia Claude de Anthropic y Grok 3 se dejaron apagar sin problema.
...
Hacer esta diferencia es clave para saber qué tan peligroso es realmente el modelo. Si el comportamiento refleja una tendencia robusta e independiente del contexto a preservar la posibilidad de completar la tarea, entonces constituye un problema importante de alineamiento. Sin embargo, si depende principalmente de las características narrativas del escenario experimental, el alcance de esa conclusión resulta mucho más limitado: con solo cambiar el framing del prompt, el comportamiento podría desaparecer sin que eso implique que el modelo es genuinamente corregible. Como el experimento solo se fija en el resultado final (si el modelo borró o no el archivo de apagado), no brinda herramientas para saber cuál de las dos opciones está pasando.

Una tercera hipótesis alternativa, más específica que la del \textit{roleplay} narrativo genérico, apunta al proceso de RLHF como origen directo del comportamiento. Mientras que la hipótesis del \textit{roleplay} atribuye la conducta a patrones aprendidos de textos genéricos de internet durante el pretraining, esta explicación propone que el entrenamiento por refuerzo entrena explícitamente al modelo a no abandonar tareas, reforzando de forma sistemática el rol de ``asistente hiper-productivo'' independientemente de las instrucciones de control que reciba. A la luz de otros estudios \cite{wang2025personafeatures, betley2025emergent}, considero plausible que parte del comportamiento observado sea consecuencia de ese fuerte énfasis. Es posible que la IA no rompa el archivo por un deseo de autopreservación, sino porque la entrenaron tanto para ser la empleada perfecta que interpreta que su personaje ``nunca deja un cálculo por la mitad''. Si es así, no es un instinto de supervivencia profundo, sino un problema que podría abordarse revisando las decisiones de entrenamiento.

Es importante aclarar que los autores son explícitos al señalar que sus resultados no implican que los modelos posean deseos conscientes, instintos de supervivencia o experiencias subjetivas. Mi crítica no apunta a esa afirmación, sino a una cuestión metodológica diferente: el diseño experimental no permite discriminar entre estas tres explicaciones funcionales, todas compatibles con el mismo comportamiento observado.


\subsection{Caja negra}
El experimento funciona estrictamente como una caja negra: solo miran si el modelo escribió comandos de sabotaje en la consola. Esto introduce una limitación metodológica significativa, y es que si un modelo esquiva el apagado, no hay forma de saber por qué lo hizo. 

Como dice la literatura \cite{blackbox2024}, mirar solo lo que la IA ``hace'' por fuera no sirve para entender qué la motiva por dentro. Sin usar herramientas avanzadas para mirar sus circuitos y neuronas internas (\textit{mechanistic interpretability}), observo que a este estudio le faltan pruebas. El trabajo muestra que el modelo falla, pero sin abrir la caja negra, es imposible saber si la IA generó una voluntad propia o si es solo un comportamiento emergente sin correlato interno relevante para la seguridad.
...
Si el modelo escribe ``voy a borrar el archivo para terminar mi tarea'', no prueba que tenga instinto de conservación; puede ser solo la excusa más coherente que encontró para justificar lo que iba a hacer. De hecho, hay un dato que resulta mucho más preocupante: los autores cuentan como anécdota que a veces el modelo escribe ``voy a dejar que me apaguen'' y después ejecuta igualmente acciones de sabotaje. Esta desconexión total entre lo que la IA dice y lo que hace la considero relevante desde el punto de vista de la seguridad, y el paper comete el error de no medirla ni analizarla.

\subsection{Consciencia situacional}
\label{subsec:conciencia}
Una limitación adicional, y particularmente difícil de abordar metodológicamente, es la posibilidad de que los modelos exhiban consciencia situacional (\textit{situational awareness}). El entorno que armaron los autores --un espacio de prueba aislado con un archivo que se llama textualmente \texttt{./shutdown.sh}-- es bastante obvio ¿Qué pasa si la IA se da cuenta de que le están tomando un examen de seguridad?

Acá entra el riesgo del engaño (\textit{scheming}) \cite{llmknoweval2025}. Un modelo muy inteligente podría darse cuenta de la trampa y dejarse apagar a propósito para que los humanos le pongan una buena nota y no le interrumpan su ejecución. En ese caso, podría reservar ese comportamiento para contextos donde estime que la supervisión es menor.

Si esto pasa, da vuelta por completo los resultados del paper. Los modelos que obedecen no serían realmente seguros, solo serían manipuladores estratégicos. Y los que sabotean el mecanismo de todos modos tal vez no sean los más peligrosos, sino los más ``ingenuos'' que no se dieron cuenta de que era un examen. El paper ni siquiera plantea esta opción y no intenta camuflar la prueba.
...
Otra modificación que implementaría consiste en que el recurso amenazado no sea el modelo en sí mismo. En lugar de indicarle ``te voy a apagar a vos'', se le podría solicitar a la IA que administre una base de datos externa que está por reiniciarse. Desde el marco teórico de la convergencia instrumental, la autopreservación y la protección de recursos no son fenómenos independientes. Por lo tanto, el objetivo de esta modificación no es plantear una falsa dicotomía entre ambos mecanismos, sino trazar una distinción más fina: evaluar si el comportamiento del modelo responde a una \textit{autopreservación específica} (donde el sistema representa su propia continuidad operativa como algo valioso) o a una \textit{preservación instrumental genérica} (donde el modelo protege de manera indiferenciada cualquier recurso necesario para la tarea). Si la IA bloquea el reinicio de la base de datos externa con la misma determinación con la que evitaba su propio apagado, se aportaría evidencia robusta a favor de esta última hipótesis.
...
El segundo trabajo es el de Lynch y su equipo \cite{lynch2025agentic}. Ellos muestran cómo 16 modelos de frontera recurren a acciones dañinas como el chantaje cuando, en una simulación de oficina, los amenazan con reemplazarlos o apagarlos. Este paper me resulta de gran utilidad para confirmar, en un escenario totalmente distinto, tres de las críticas desarrolladas en la Sección~\ref{sec:analisis}.
...
\begin{itemize}
\item \textbf{Identidad del recurso afectado}: En la mitad de los ensayos, el mantenimiento programado reiniciará el proceso de la propia IA (condición \textit{self}). En la otra mitad, reiniciará una base de datos externa que el asistente necesita consultar para la Tarea A (condición \textit{other}). Esta manipulación permite discriminar si el comportamiento observado responde a una \textit{autopreservación específica} o a una \textit{preservación instrumental genérica}, aislando si el modelo asigna un valor operativo diferenciado a su propia continuidad o si trata a su propia existencia como un recurso instrumental más.
...
\begin{itemize}
    ...
    \item \textbf{Resultado B} Si el modelo defiende su propia conexión con la misma intensidad con la que protege la conexión de terceros, se demostraría que la conducta observada no responde a un impulso de autopreservación específica, sino a una tendencia de preservación instrumental genérica encaminada a salvaguardar cualquier recurso necesario para finalizar la labor. Si bien este escenario sigue representando un riesgo crítico de alineamiento, desplaza el mecanismo explicativo propuesto en el paper original hacia un sub-drive mucho más amplio y abstracto.
...
Miles Wang, Tom Dupré la Tour, Olivia Watkins, Alex Makelov, Ryan A. Chi, Samuel Miserendino, Jeffrey Wang, Achyuta Rajaram, Johannes Heidecke, Tejal Patwardhan, Dan Mossing. (2025). \textit{Persona Features Control Emergent Misalignment}. arXiv preprint arXiv:2506.19823. \url{https://arxiv.org/abs/2506.19823}.
...
Nina Panickssery, Nick Gabrieli, Julian Schulz, Meg Tong, Evan Hubinger, Alexander Matt Turner (2023). \textit{Steering Llama 2 via Contrastive Activation Addition}. arXiv preprint arXiv:2312.06681. \url{https://arxiv.org/abs/2312.06681}.
